...
def sliding_window2(image, window_size, scale, stride):
    windows_number = 50
    ...
    patches = np.zeros((window_rows, window_cols,windows_number));
    bbox_locations = np.zeros((windows_number,4))
    r = np.random.randint(0,image_rows-window_rows,windows_number); # Sample top left position
    #devuelve 5 numeros random desde 0 a image_rows-window_rows
    c = np.random.randint(0,image_cols-window_cols,windows_number);
